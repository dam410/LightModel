...
function [norm_L2,gradient_norm_L2] = evaluate_error_homography_monotone_L2(I,pt_in,I_pt,I_vect,H,a_vect)
	n_pt = length(I_pt);
	n_vect = length(a_vect);
	if nargout > 1
		[err,J_err_full] = evaluate_error_homography_monotone(I,pt_in,I_pt,I_vect,H,a_vect);
		[norm_L2,gradient_norm_L2] = error_norm_L2(err,J_err_full);
	else
