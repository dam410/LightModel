function [dssp,psp,ps] = case_8_np(data,groundtruth)

	EPS = 1e-8;
	scale = norm(groundtruth.SourcePosition);
	...
	Xcs = cell(1,n_p);
	Ns = cell(1,n_p);
	% Calculate the angle between z-vector and groundtruth angle
	angle_z_vect = zeros(1,n_p);
	angle_orient = zeros(1,n_p);
	for i_p = 1:n_p
		...
		if size(N,2)==2
			...
			%disp('N Xc Ps N_gt angle');
			%N(:,1)
			%Xc(:,1)
			%P_s_1(:,1)
			%groundtruth.ScenePlaneOrientation{i_p}(:,3)
			%180*acos([1,0,0]*P_s_1(1:3,1))/pi
			%angle_z_vect(i_p) = 180*acos([0,0,1]*groundtruth.ScenePlaneOrientation{i_p}(:,3))/pi;
			%angle_orient(i_p) = 180*acos([N(1,1),N(2,1),N(3,1)]*groundtruth.ScenePlaneOrientation{i_p}(:,3))/pi;
			dssp{i_p} = {Inf};
			psp{i_p} = {[N(:,1);0],[N(:,2);0]};
	...
	%figure('Name','Display the angle between estimated normal and groundtruth depending on plane inclination');
	%plot(angle_orient,angle_z_vect,'+r');
